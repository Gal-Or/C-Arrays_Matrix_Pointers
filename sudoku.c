...
{	//check if every number appears one time only in the row
	...
	for(int i = 0; i<size ; i++)
	{	
		if(*(tempArr+1+*(matrix+i)) == '1' || *(matrix+i)>size || *(matrix+i)< 1) 
			return 0;
		else 
...
{	////check if every number appears one time only in the square
	...
	for(int i =0; i<sqrtNum ; i++)
	{
		...
		{
			if(*(tempArr+1+*(matrix+i*size+j)) == '1' || *(matrix+i*size+j) > size || *(matrix+i*size+j) < 1)
				return 0;
			else
...
{	//send every square to check
	...
	{	for(int j =0 ; j<sqrtNum; j++)
		{
			memset(tempArr, '0', MAT_SIZE+1);
			int ok = checkOneSquare(matrix+i*size*sqrtNum+j*sqrtNum, size, tempArr);
			if(ok == 0)
				return 0 ;
		}
	}
...
{	//check the board by methods 
	...
	if(okCols == 0 )
		return 0;
	int okSquares = checkSquares(matrix, size, tempArr);
	if(okSquares == 0)
		return 0;
...
{	//main method that start the checking of the board
	...
	if(ok ==0)
		printf("\nSudoku board is incorrect!");
	else
		printf("\nSudoku board is correct!");
}
